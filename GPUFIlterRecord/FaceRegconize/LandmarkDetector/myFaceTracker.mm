...
//==============================================================================
void myTrackFunc1(cv::Mat &/*im*/,   //image containing object
             cv::Mat &s,        //current shape describing object
             cv::Mat &dxdp,     //shape model jacobian at shape
             cv::Mat &H,        //contains Hessian on return
             cv::Mat &g,        //contains gradient on return
...
//==============================================================================
void myTrackFunc2(cv::Mat &/*im*/,   //image containing object
             cv::Mat &s,    //current shape describing object
             cv::Mat &dxdp, //shape model jacobian at shape
             cv::Mat &H,    //contains Hessian on return
             cv::Mat &g,    //contains gradient on return
...
int myFaceTracker::trackerWithRect(cv::Mat &im, FaceTrackerParams * params ,cv::Rect rect)
{
    ...
    if(!serialQueue){
         serialQueue = dispatch_queue_create("faceCamera.serialQueue", DISPATCH_QUEUE_SERIAL);
    }
    if (p->track_type > 0) {
        ...
        e1 = cv::getTickCount();
        __block cv::Mat pose = _clm._pglobl(cv::Rect(0,1,1,3));
        if (!_atm._init) {
            //            if (_atm._scale == 1) {
            //                smooth_ = gray_;
            //            } else {
...
//        e2 = cv::getTickCount();
//        float t = (e2 - e1)/cv::getTickFrequency();
//        printf(" _atm.Update %f \n",t );
//        e1 = cv::getTickCount();
//        
//        e2 = cv::getTickCount();
//        t = (e2 - e1)/cv::getTickFrequency();
//        printf(" Track CalcJacob %f \n",t );
        // if(_pra._pra.size() > 0){
        //   if(p->track_type > 1){
        // 	if(!_ksmooth._init)
